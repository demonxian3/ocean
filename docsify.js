(function () {
  ...
  Compiler.prototype._initRenderer = function _initRenderer() {
    ...
    origin.listitem = renderer.listitem = function (text) {
      ...
      if ( (parts = />(.+?)<\/a>/g.exec(text)) && parts[1] == getCookie('curSidebar')) {
        collapse = '';
      }
      var html = isTaskItem ? ("<li class=\"task-list-item\"><label>" + text + "</label></li>") : (`<li class='${collapse}'>${text}</li>`);
      return html
    };
  ...
  function collapse(el) {
    ...
    on(el, 'click', function (ref) {
      ...
      if (
        ...
        setCookie('curSidebar', target.parentNode.innerText.trim());
